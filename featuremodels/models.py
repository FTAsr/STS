...
import sys
sys.path = ['../feedbackmodels'] + sys.path
from feedbackmodels import models as mod
sys.path = ['../utils', '../monolingual-word-aligner'] + sys.path
import utils
...
class align(object):
    ...
    def __init__(self):
        ...
        self.bow = bow('../pretrained/embeddings/GoogleNews-vectors-negative300.bin')
        return
...
class feedback(object):
    """
    Infersent model embeddings
    """
    feedback_model = None
    
    def __init__(self):
        print("feedback init: loading the pretrained model")
        self.feedback_model = mod.loadFeedbackModel()
        return

    def sentence_similarity(self, sentenceA, sentenceB):
        #features = []
        dist = cosine(self.feedback_model.encode([sentenceA])[0], self.feedback_model.encode([sentenceB])[0])
    ...
    def pairFeatures(self, sentenceA, sentenceB):
        #obtains the distributional representation of each sentence 
        #generate element-wise subtraction and multiplication features
        a = self.feedback_model.encode([sentenceA])
        b = self.feedback_model.encode([sentenceB])
        f = np.c_[np.abs(a - b), a * b]
        #print len(f[0]), " word2vec features generated"
        return f[0]
