...
if __name__ == '__main__':
    ...
    '''
    bowclassifier = pickle.load(open('super.file', 'rb'))
    fbclassifier = pickle.load(open('fb.file', 'rb'))
    aligner = pickle.load(open('align.file', 'rb'))
    '''

    ## Test the classifier on test data of the same type (coming from SICK)
    testbow = test([bowm], classifiers[0], testSet)['prediction']
    testfb = test([fbm], classifiers[1], testSet)['prediction']
    testalign = test([alignm], classifiers[2], testSet)
    testaligner = testalign['prediction']
    score = testalign['score']
