...
#
import sys 
sys.path.append('../pretrained/embeddings') 
sys.path.append('../feedbackmodels')
sys.path.append('../') 
...
class BLSTMEncoder(nn.Module):
    ...
    def visualize(self, sent, tokenize=True, threshold=10):
        if tokenize: from nltk.tokenize import word_tokenize
        ...
        plt.show()
        num = int(np.floor(len(y)/2.0))
        index = sorted(range(len(y)), key=lambda i: y[i])[-num:]
        #index = [i for i,x in enumerate(y) if x > threshold]
        keywords = [sent[0][i].lower() for i in index]
        #print keywords
        #return output, idxs
        return keywords
                             
def loadFeedbackModel():
    feedbackmod = torch.load('../feedbackmodels/infersent.allnli.pickle')
    ...
    feedbackmod.set_glove_path('../pretrained/embeddings/glove.840B.300d.txt')
    #feedbackmod.build_vocab_k_words(K=500000)
    return feedbackmod

def feedback(feedbackmod, goldans, threshold):
    return feedbackmod.visualize(goldans, threshold)
